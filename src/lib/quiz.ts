...
const TYPE_META: Record<string, { keywords: string[]; en: string; img: string; tagline: string; cpTypes: string[]; worstTypes: string[]; roast: string; hype: string; action: string; profile: string; emoji: string }> = {
  ...
  '30km废物': {
    ...
    tagline: '30公里只是热身，35才是开始',
    cpTypes: ['补给守财奴', '退赛钉子户'],
    worstTypes: ['配速舔狗', 'Strava装逼犯'],
  ...
  '补给站强盗': {
    ...
    tagline: '补给站才是终点',
    cpTypes: ['30km废物', '赛道遛弯大爷'],
    worstTypes: ['社恐跑者', '赤脚大仙'],
  ...
  '数据奴隶': {
    ...
    tagline: '手表说今天状态不好，那就不好',
    cpTypes: ['Strava装逼犯', '配速舔狗'],
    worstTypes: ['佛系废物', '赛道遛弯大爷'],
  ...
  '购物车跑者': {
    ...
    tagline: '装备买齐了，人还没齐',
    cpTypes: ['数据奴隶', 'Strava装逼犯'],
    worstTypes: ['赤脚大仙', '佛系废物'],
  ...
  '当年勇选手': {
    ...
    tagline: '想当年我首马3xx',
    cpTypes: ['跑圈NPC', '30km废物'],
    worstTypes: ['Strava装逼犯', '配速舔狗'],
  ...
  '爬坡废物': {
    ...
    cpTypes: ['赛道遛弯大爷', '补给站强盗'],
    worstTypes: ['补给守财奴', '配速舔狗'],
  ...
  '5点变态': {
    ...
    tagline: '5点跑步8点打卡上班',
    cpTypes: ['配速舔狗', 'Strava装逼犯'],
    worstTypes: ['躺完选手', '佛系废物'],
  ...
  '火锅赎罪券': {
    ...
    cpTypes: ['补给站强盗', '赛道遛弯大爷'],
    worstTypes: ['数据奴隶', '补给守财奴'],
...
function determineRunnerType(answers: Answer[], dimensionScores: Record<string, number>): string {
  ...
  return MACRO_TYPE_TABLE[code] || '佛系废物'
}
...
const bestCpDescs: Record<string, string> = {
    '社恐跑者': '你们之间不需要对话。一个发路线截图，另一个回OK，全程不说话，跑完各自回家。完美的社交距离，完美的配速默契。唯一的问题是——你们谁都不会先开口约下一次。',
    '奖牌舔狗': '你们赛前都睡不着但原因不同——你是因为紧张他是因为兴奋。你们在终点线会师互相举着手机拍照，友谊建立在完赛证书之上。你们约跑的动力不是训练是收集下一块奖牌。',
    '朋友圈跑者': '你们跑步的节奏完美同步——你停下来喝水他停下来拍照，你加速他在找下一个构图点。你们都不在乎配速，在乎的是这条路线好不好看。你们的合照永远是赛道上最好看的。',
    '30km废物': '你们30公里后的配速完全一致——都是走路。但走得很默契，互相递能量胶互相说"加油就快了"。你们的完赛时间可能比跑完的还快乐，因为后半程你们一直在聊天。',
    '报名鸽王': '你们互相理解"报名就是参与"的哲学。你说"下次一定"他说"我也一定"，然后你们一起鸽了那场比赛。你们的友谊不需要赛道证明，报名截图就够了。',
    '补给站强盗': '你们的比赛节奏是跑到补给站、吃到关门、跑向下一个补给站。你知道哪个站有西瓜他知道哪个站有可乐。你们是赛道上的美食搭档，配速不重要，补给的性价比才重要。',
    '数据奴隶': '你们交换手表数据的热情比跑步本身还高，跑完第一件事不是喝水是看VO2max。你们能就"今天的训练效果是3.8还是4.0"争论一整个下午。',
    ...
    '佛系废物': '你们都说"看心情"然后一起不跑。偶尔约跑但每次都被"今天不太想动"打败。你们的友谊不需要赛道证明，一个"明天跑吗？"和一个"再说吧"就够了。你们是跑圈最稳定的关系——因为从不跑步所以从不吵架。',
    '配速舔狗': '你们都盯着配速跑，谁快了慢了都要互相提醒。约跑是精确到秒的训练课，聊天内容是VO2max和乳酸阈值。你们是赛道上的"数据二人组"，别人跑步靠感觉你们靠手表。',
  }
...
function buildCpMatch(runnerType: string) {
  ...
  if (!meta || !meta.cpTypes.length) {
    return { type: '奖牌舔狗', desc: '随机匹配，看缘分。', score: 75 }
  }
...
function buildWorstCpMatch(runnerType: string) {
  ...
  const worstDescs: Record<string, string> = {
    '社恐跑者': '他非要拉你进跑团，你只想一个人跑。他发消息约跑，你假装没看到。',
    '奖牌舔狗': '他跑完要发朋友圈，你只想回家。他觉得你不在乎，你觉得他太装。',
    '朋友圈跑者': '他停下来拍照，你配速被打乱。你等他构图，他觉得你不配合。',
    '30km废物': '他30km开始走，你在后面纠结要不要陪。陪了慢，不陪不义。',
    '报名鸽王': '他每次报名但不来，你替他惋惜。下次他还不来，你已经不想约了。',
    '补给站强盗': '他每个补给站都停，你等得脚冷。你说走吧，他说再吃一口。',
    '数据奴隶': '他看手表比看你多，你说今天感觉不错，他说配速掉了5秒。',
    ...
    '佛系废物': '他说跑不跑都行你已穿好鞋，他总鸽你还每次都约。他的"看心情"翻译过来就是"不跑"，你的认真在他的佛系面前变成了内卷。你们的关系就像一个永远在等一个永远不来。',
    '配速舔狗': '他看配速跑步，你凭感觉。他觉得你太慢，你觉得他太卷。',
  }
...
export function getSuitableRace(runnerType: string): string {
  const races: Record<string, string> = {
    '社恐跑者': 'UTMB（一个人跑完全程，才是真正的朝圣）',
    '奖牌舔狗': '北京马拉松/上海马拉松（国马/上马，完赛证书最具仪式感）',
    '朋友圈跑者': '厦门马拉松/杭州马拉松（海滨+西湖，赛道就是你的影棚）',
    '30km废物': '半马（缩短战线，体验一次全程不停脚的快感）',
    '报名鸽王': '任何一场报名费低廉的赛事（押金不能白交）',
    '补给站强盗': '成都马拉松/广州马拉松（补给天花板，美食赛道）',
    '数据奴隶': '香港100（数据党的朝圣之路，CP点精确到秒）',
...
export function getRaceRecommendations(runnerType: string): Array<{
  ...
  const typeTags: Record<string, string[]> = {
    '社恐跑者': ['UTMB', '巨人之旅', '贡嘎100', '梅里100'],
    '奖牌舔狗': ['北京马拉松', '上海马拉松', 'HK100', '崇礼168', '江南百英里'],
    '朋友圈跑者': ['厦门马拉松', '杭州马拉松', '四姑娘山', '武功山', '张家界天门山', '大理马拉松'],
    '30km废物': ['千岛湖马拉松', '郑开马拉松', '莫干山', '江南百英里'],
    '报名鸽王': ['海口马拉松', '莫干山', '珠海马拉松'],
    '补给站强盗': ['成都马拉松', '广州马拉松', '武汉马拉松', '长沙马拉松', '重庆马拉松', '哈尔滨马拉松'],
    '数据奴隶': ['HK100', '无锡马拉松', '上海马拉松'],
    ...
    '佛系废物': ['杭州马拉松', '四姑娘山', '大理100', '武功山', '丽江马拉松'],
    '配速舔狗': ['无锡马拉松', '上海马拉松', '北京马拉松', '蜀道by UTMB'],
  }
